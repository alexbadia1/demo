import {
  ISchemaTemplates,
  IService,
  IServicePair,
} from '@/redux/designer/models';
import _ from 'lodash';

const AWS_SCHEMAS: ISchemaTemplates = {
  'Security-Identity-Compliance': {
    'Directory-Service': {
      id: 'Directory-Service',
      category: 'Security-Identity-Compliance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Identity-and-Access-Management': {
      id: 'Identity-and-Access-Management',
      category: 'Security-Identity-Compliance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    CloudHSM: {
      id: 'CloudHSM',
      category: 'Security-Identity-Compliance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Verified-Permissions': {
      id: 'Verified-Permissions',
      category: 'Security-Identity-Compliance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    GuardDuty: {
      id: 'GuardDuty',
      category: 'Security-Identity-Compliance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Detective: {
      id: 'Detective',
      category: 'Security-Identity-Compliance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Inspector: {
      id: 'Inspector',
      category: 'Security-Identity-Compliance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Cloud-Directory': {
      id: 'Cloud-Directory',
      category: 'Security-Identity-Compliance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Signer: {
      id: 'Signer',
      category: 'Security-Identity-Compliance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Key-Management-Service': {
      id: 'Key-Management-Service',
      category: 'Security-Identity-Compliance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Firewall-Manager': {
      id: 'Firewall-Manager',
      category: 'Security-Identity-Compliance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Audit-Manager': {
      id: 'Audit-Manager',
      category: 'Security-Identity-Compliance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Network-Firewall': {
      id: 'Network-Firewall',
      category: 'Security-Identity-Compliance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Secrets-Manager': {
      id: 'Secrets-Manager',
      category: 'Security-Identity-Compliance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Macie: {
      id: 'Macie',
      category: 'Security-Identity-Compliance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Cognito: {
      id: 'Cognito',
      category: 'Security-Identity-Compliance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Shield: {
      id: 'Shield',
      category: 'Security-Identity-Compliance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Security-Hub': {
      id: 'Security-Hub',
      category: 'Security-Identity-Compliance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    WAF: {
      id: 'WAF',
      category: 'Security-Identity-Compliance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Private-Certificate-Authority': {
      id: 'Private-Certificate-Authority',
      category: 'Security-Identity-Compliance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Security-Lake': {
      id: 'Security-Lake',
      category: 'Security-Identity-Compliance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Artifact: {
      id: 'Artifact',
      category: 'Security-Identity-Compliance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Resource-Access-Manager': {
      id: 'Resource-Access-Manager',
      category: 'Security-Identity-Compliance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'IAM-Identity-Center': {
      id: 'IAM-Identity-Center',
      category: 'Security-Identity-Compliance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Certificate-Manager': {
      id: 'Certificate-Manager',
      category: 'Security-Identity-Compliance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
  },
  'Business-Applications': {
    Honeycode: {
      id: 'Honeycode',
      category: 'Business-Applications',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Supply-Chain': {
      id: 'Supply-Chain',
      category: 'Business-Applications',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Wickr: {
      id: 'Wickr',
      category: 'Business-Applications',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    WorkDocs: {
      id: 'WorkDocs',
      category: 'Business-Applications',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Chime-SDK': {
      id: 'Chime-SDK',
      category: 'Business-Applications',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'WorkDocs-SDK': {
      id: 'WorkDocs-SDK',
      category: 'Business-Applications',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Pinpoint: {
      id: 'Pinpoint',
      category: 'Business-Applications',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Simple-Email-Service': {
      id: 'Simple-Email-Service',
      category: 'Business-Applications',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Alexa-For-Business': {
      id: 'Alexa-For-Business',
      category: 'Business-Applications',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Chime-Voice-Connector': {
      id: 'Chime-Voice-Connector',
      category: 'Business-Applications',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Chime: {
      id: 'Chime',
      category: 'Business-Applications',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    WorkMail: {
      id: 'WorkMail',
      category: 'Business-Applications',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Pinpoint-APIs': {
      id: 'Pinpoint-APIs',
      category: 'Business-Applications',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Connect: {
      id: 'Connect',
      category: 'Business-Applications',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
  },
  Games: {
    'Open-3D-Engine': {
      id: 'Open-3D-Engine',
      category: 'Games',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Lumberyard: {
      id: 'Lumberyard',
      category: 'Games',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    GameSparks: {
      id: 'GameSparks',
      category: 'Games',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    GameLift: {
      id: 'GameLift',
      category: 'Games',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    GameKit: {
      id: 'GameKit',
      category: 'Games',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
  },
  'VR-AR': {
    Sumerian: {
      id: 'Sumerian',
      category: 'VR-AR',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
  },
  'End-User-Computing': {
    WorkLink: {
      id: 'WorkLink',
      category: 'End-User-Computing',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'WorkSpaces-Family': {
      id: 'WorkSpaces-Family',
      category: 'End-User-Computing',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    AppStream: {
      id: 'AppStream',
      category: 'End-User-Computing',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
  },
  Storage: {
    'FSx-for-WFS': {
      id: 'FSx-for-WFS',
      category: 'Storage',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Snowball-Edge': {
      id: 'Snowball-Edge',
      category: 'Storage',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'FSx-for-OpenZFS': {
      id: 'FSx-for-OpenZFS',
      category: 'Storage',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Simple-Storage-Service': {
      id: 'Simple-Storage-Service',
      category: 'Storage',
      description:
        'Amazon S3 (Simple Storage Service) provides scalable object storage (similar to Google Drive).',
      enabled: true,
      cost: {
        flat: 0,
        'per-user': 0.0046,
      },
      config: {
        versioning: {
          title: 'Versioning',
          hint: 'Enable versioning for your S3 bucket',
          type: 'boolean',
          value: false,
          required: false,
        },
        bucket_name: {
          title: 'Bucket Name',
          hint: 'Name of your component',
          type: 'string',
          value: 'bucket1',
          required: false,
        },
      },
      deploy: {
        build_directory: {
          title: 'Build Directory',
          hint: 'Location of build files in your repository',
          type: 'string',
          value: '/build',
          required: false,
        },
      },
      neighbors: [],
      onConnect: (mySchema: IService, targetSchema: IService) => {
        const result: IServicePair = {};

        if (targetSchema.id === 'CloudFront') {
          targetSchema.config.bucket_name.value =
            mySchema.config.bucket_name.value;
          targetSchema.config.bucket_name.readonly = true;
          result.target = targetSchema;
        }

        return result;
      },
      onDisconnect: (mySchema: IService, targetSchema: IService) => {
        const result: IServicePair = {};

        if (targetSchema.id === 'CloudFront') {
          targetSchema.config.bucket_name.value = '';
          targetSchema.config.bucket_name.readonly = false;
          result.target = targetSchema;
        }

        return result;
      },
      onChange: (mySchema: IService, connectedSchema: IService) => {
        const result: IServicePair = {};

        if (connectedSchema.id === 'CloudFront') {
          connectedSchema.config.bucket_name.value =
            mySchema.config.bucket_name.value;
          connectedSchema.config.bucket_name.readonly = true;
          result.target = connectedSchema;
        }

        return result;
      },
    },
    'File-Cache': {
      id: 'File-Cache',
      category: 'Storage',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Backup: {
      id: 'Backup',
      category: 'Storage',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'FSx-for-NetApp-ONTAP': {
      id: 'FSx-for-NetApp-ONTAP',
      category: 'Storage',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Snowmobile: {
      id: 'Snowmobile',
      category: 'Storage',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Elastic-Disaster-Recovery': {
      id: 'Elastic-Disaster-Recovery',
      category: 'Storage',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Snowcone: {
      id: 'Snowcone',
      category: 'Storage',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    FSx: {
      id: 'FSx',
      category: 'Storage',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Snowball: {
      id: 'Snowball',
      category: 'Storage',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'FSx-for-Lustre': {
      id: 'FSx-for-Lustre',
      category: 'Storage',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    EFS: {
      id: 'EFS',
      category: 'Storage',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Storage-Gateway': {
      id: 'Storage-Gateway',
      category: 'Storage',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Simple-Storage-Service-Glacier': {
      id: 'Simple-Storage-Service-Glacier',
      category: 'Storage',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Elastic-Block-Store': {
      id: 'Elastic-Block-Store',
      category: 'Storage',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'S3-on-Outposts': {
      id: 'S3-on-Outposts',
      category: 'Storage',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
  },
  'Networking-Content-Delivery': {
    'Client-VPN': {
      id: 'Client-VPN',
      category: 'Networking-Content-Delivery',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Private-5G': {
      id: 'Private-5G',
      category: 'Networking-Content-Delivery',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Cloud-WAN': {
      id: 'Cloud-WAN',
      category: 'Networking-Content-Delivery',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    CloudFront: {
      id: 'CloudFront',
      category: 'Networking-Content-Delivery',
      description:
        'Amazon CloudFront is a web service for content delivery. It integrates with other Amazon Web Services to give developers and businesses an easy way to distribute content to end users with low latency, high data transfer speeds, and no commitments.',
      enabled: true,
      cost: {
        flat: 0,
        'per-user': 0.017,
      },
      config: {
        name: {
          title: 'Name',
          hint: 'Name for your CloudFront distribution',
          type: 'string',
          value: 'cloudfront',
          required: true,
        },
        bucket_name: {
          title: 'S3 Origin',
          hint: 'Origin for your CloudFront distribution',
          type: 'string',
          value: 'bucket1',
          required: true,
        },
      },
      deploy: {},
      neighbors: [],
    },
    'private subnet': {
      id: 'private subnet',
      category: 'Networking-Content-Delivery',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'public subnet': {
      id: 'public subnet',
      category: 'Networking-Content-Delivery',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Route-53': {
      id: 'Route-53',
      category: 'Networking-Content-Delivery',
      description:
        'Amazon Route 53 is a scalable and highly available Domain Name System (DNS) web service.',
      enabled: true,
      cost: {
        flat: 0.5,
        'per-user': 0.0,
      },
      config: {
        domain: {
          title: 'Domain Name',
          hint: 'Domain name for your Route 53 hosted zone (excluding http:// or www)',
          type: 'string',
          value: 'stratus.com',
          required: true,
        },
      },
      deploy: {},
      neighbors: [],
    },
    'Route-53 Route-Table': {
      id: 'Route-53 Route-Table',
      category: 'Networking-Content-Delivery',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Virtual-Private-Cloud': {
      id: 'Virtual-Private-Cloud',
      category: 'Networking-Content-Delivery',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Cloud-Directory': {
      id: 'Cloud-Directory',
      category: 'Networking-Content-Delivery',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'VPC-Lattice': {
      id: 'VPC-Lattice',
      category: 'Networking-Content-Delivery',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Direct-Connect': {
      id: 'Direct-Connect',
      category: 'Networking-Content-Delivery',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Verified-Access': {
      id: 'Verified-Access',
      category: 'Networking-Content-Delivery',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'App-Mesh': {
      id: 'App-Mesh',
      category: 'Networking-Content-Delivery',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Site-to-Site-VPN': {
      id: 'Site-to-Site-VPN',
      category: 'Networking-Content-Delivery',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    PrivateLink: {
      id: 'PrivateLink',
      category: 'Networking-Content-Delivery',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Cloud-Map': {
      id: 'Cloud-Map',
      category: 'Networking-Content-Delivery',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Transit-Gateway': {
      id: 'Transit-Gateway',
      category: 'Networking-Content-Delivery',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Elastic-Load-Balancing': {
      id: 'Elastic-Load-Balancing',
      category: 'Networking-Content-Delivery',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Global-Accelerator': {
      id: 'Global-Accelerator',
      category: 'Networking-Content-Delivery',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
  },
  Analytics: {
    'Kinesis-Data-Streams': {
      id: 'Kinesis-Data-Streams',
      category: 'Analytics',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Data-Pipeline': {
      id: 'Data-Pipeline',
      category: 'Analytics',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Data-Exchange': {
      id: 'Data-Exchange',
      category: 'Analytics',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Redshift: {
      id: 'Redshift',
      category: 'Analytics',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Kinesis: {
      id: 'Kinesis',
      category: 'Analytics',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Kinesis-Video-Streams': {
      id: 'Kinesis-Video-Streams',
      category: 'Analytics',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Managed-Streaming-for-Apache-Kafka': {
      id: 'Managed-Streaming-for-Apache-Kafka',
      category: 'Analytics',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Athena: {
      id: 'Athena',
      category: 'Analytics',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Glue-Elastic-Views': {
      id: 'Glue-Elastic-Views',
      category: 'Analytics',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    CloudSearch: {
      id: 'CloudSearch',
      category: 'Analytics',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Kinesis-Data-Analytics': {
      id: 'Kinesis-Data-Analytics',
      category: 'Analytics',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Lake-Formation': {
      id: 'Lake-Formation',
      category: 'Analytics',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    QuickSight: {
      id: 'QuickSight',
      category: 'Analytics',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Kinesis-Firehose': {
      id: 'Kinesis-Firehose',
      category: 'Analytics',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    DataZone: {
      id: 'DataZone',
      category: 'Analytics',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Clean-Rooms': {
      id: 'Clean-Rooms',
      category: 'Analytics',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Glue: {
      id: 'Glue',
      category: 'Analytics',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'OpenSearch-Service': {
      id: 'OpenSearch-Service',
      category: 'Analytics',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    EMR: {
      id: 'EMR',
      category: 'Analytics',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Glue-DataBrew': {
      id: 'Glue-DataBrew',
      category: 'Analytics',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    FinSpace: {
      id: 'FinSpace',
      category: 'Analytics',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
  },
  Database: {
    'RDS-on-VMware': {
      id: 'RDS-on-VMware',
      category: 'Database',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Keyspaces: {
      id: 'Keyspaces',
      category: 'Database',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'MemoryDB-for-Redis': {
      id: 'MemoryDB-for-Redis',
      category: 'Database',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Timestream: {
      id: 'Timestream',
      category: 'Database',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Neptune: {
      id: 'Neptune',
      category: 'Database',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    RDS: {
      id: 'RDS',
      category: 'Database',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    DynamoDB: {
      id: 'DynamoDB',
      category: 'Database',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    DocumentDB: {
      id: 'DocumentDB',
      category: 'Database',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Aurora: {
      id: 'Aurora',
      category: 'Database',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    ElastiCache: {
      id: 'ElastiCache',
      category: 'Database',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Database-Migration-Service': {
      id: 'Database-Migration-Service',
      category: 'Database',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
  },
  'Developer-Tools': {
    CodeCommit: {
      id: 'CodeCommit',
      category: 'Developer-Tools',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Cloud-Development-Kit': {
      id: 'Cloud-Development-Kit',
      category: 'Developer-Tools',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Application-Composer': {
      id: 'Application-Composer',
      category: 'Developer-Tools',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Corretto: {
      id: 'Corretto',
      category: 'Developer-Tools',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    CodeArtifact: {
      id: 'CodeArtifact',
      category: 'Developer-Tools',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Command-Line-Interface': {
      id: 'Command-Line-Interface',
      category: 'Developer-Tools',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Cloud-Control-API': {
      id: 'Cloud-Control-API',
      category: 'Developer-Tools',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    CodeDeploy: {
      id: 'CodeDeploy',
      category: 'Developer-Tools',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'X-Ray': {
      id: 'X-Ray',
      category: 'Developer-Tools',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    CodePipeline: {
      id: 'CodePipeline',
      category: 'Developer-Tools',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    CodeStar: {
      id: 'CodeStar',
      category: 'Developer-Tools',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    CodeCatalyst: {
      id: 'CodeCatalyst',
      category: 'Developer-Tools',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    CodeBuild: {
      id: 'CodeBuild',
      category: 'Developer-Tools',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Cloud9: {
      id: 'Cloud9',
      category: 'Developer-Tools',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Tools-and-SDKs': {
      id: 'Tools-and-SDKs',
      category: 'Developer-Tools',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    CloudShell: {
      id: 'CloudShell',
      category: 'Developer-Tools',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
  },
  'Internet-of-Things': {
    'IoT-ExpressLink': {
      id: 'IoT-ExpressLink',
      category: 'Internet-of-Things',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'IoT-TwinMaker': {
      id: 'IoT-TwinMaker',
      category: 'Internet-of-Things',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'IoT-Analytics': {
      id: 'IoT-Analytics',
      category: 'Internet-of-Things',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'IoT-Core': {
      id: 'IoT-Core',
      category: 'Internet-of-Things',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'IoT-Device-Management': {
      id: 'IoT-Device-Management',
      category: 'Internet-of-Things',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'IoT-FleetWise': {
      id: 'IoT-FleetWise',
      category: 'Internet-of-Things',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'IoT-Device-Defender': {
      id: 'IoT-Device-Defender',
      category: 'Internet-of-Things',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'IoT-Greengrass': {
      id: 'IoT-Greengrass',
      category: 'Internet-of-Things',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'IoT-EduKit': {
      id: 'IoT-EduKit',
      category: 'Internet-of-Things',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'IoT-RoboRunner': {
      id: 'IoT-RoboRunner',
      category: 'Internet-of-Things',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'IoT-Events': {
      id: 'IoT-Events',
      category: 'Internet-of-Things',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'IoT-1-Click': {
      id: 'IoT-1-Click',
      category: 'Internet-of-Things',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    FreeRTOS: {
      id: 'FreeRTOS',
      category: 'Internet-of-Things',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'IoT-Button': {
      id: 'IoT-Button',
      category: 'Internet-of-Things',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'IoT-Things-Graph': {
      id: 'IoT-Things-Graph',
      category: 'Internet-of-Things',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'IoT-SiteWise': {
      id: 'IoT-SiteWise',
      category: 'Internet-of-Things',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
  },
  Containers: {
    'ECS-Anywhere': {
      id: 'ECS-Anywhere',
      category: 'Containers',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Red-Hat-OpenShift-Service-on-AWS': {
      id: 'Red-Hat-OpenShift-Service-on-AWS',
      category: 'Containers',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Elastic-Container-Service': {
      id: 'Elastic-Container-Service',
      category: 'Containers',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'EKS-Cloud': {
      id: 'EKS-Cloud',
      category: 'Containers',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Elastic-Kubernetes-Service': {
      id: 'Elastic-Kubernetes-Service',
      category: 'Containers',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'EKS-Distro': {
      id: 'EKS-Distro',
      category: 'Containers',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Elastic-Container-Registry': {
      id: 'Elastic-Container-Registry',
      category: 'Containers',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Fargate: {
      id: 'Fargate',
      category: 'Containers',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'EKS-Anywhere': {
      id: 'EKS-Anywhere',
      category: 'Containers',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
  },
  Robotics: {
    RoboMaker: {
      id: 'RoboMaker',
      category: 'Robotics',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
  },
  Compute: {
    'Thinkbox-Deadline': {
      id: 'Thinkbox-Deadline',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Elastic-Fabric-Adapter': {
      id: 'Elastic-Fabric-Adapter',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'NICE-DCV': {
      id: 'NICE-DCV',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Batch: {
      id: 'Batch',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'EC2-Image-Builder': {
      id: 'EC2-Image-Builder',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'EC2-Spot-Instance': {
      id: 'EC2-Spot-Instance',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Local-Zones': {
      id: 'Local-Zones',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'App-Runner': {
      id: 'App-Runner',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Outposts-rack': {
      id: 'Outposts-rack',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    ParallelCluster: {
      id: 'ParallelCluster',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Outposts-family': {
      id: 'Outposts-family',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Nitro-Enclaves': {
      id: 'Nitro-Enclaves',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Outposts-servers': {
      id: 'Outposts-servers',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Thinkbox-Frost': {
      id: 'Thinkbox-Frost',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Thinkbox-Krakatoa': {
      id: 'Thinkbox-Krakatoa',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Compute-Optimizer': {
      id: 'Compute-Optimizer',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Lambda: {
      id: 'Lambda',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {
        function_name: {
          title: 'Function Name',
          hint: 'Name for your Lambda function',
          type: 'string',
          value: 'lambda1',
          required: true,
        },
        memory_size: {
          title: 'Memory Size (MB)',
          hint: 'The amount of memory for the function',
          type: 'number_range',
          value: 128,
          min: 128,
          max: 10_240,
          required: true,
        },
        ephemeral_storage: {
          title: 'Ephemeral Storage (MB)',
          hint: 'The amount of ephemeral storage for the function',
          type: 'number_range',
          value: 512,
          min: 512,
          max: 1_024,
          required: true,
        },
        timeout: {
          title: 'Timeout (Seconds)',
          hint: 'The timeout for the function',
          type: 'number_range',
          value: 3,
          min: 3,
          max: 900,
          required: true,
        },
        maximum_retry_attempts: {
          title: 'Maximum Retry Attempts',
          hint: 'The maximum retry attempts for the function',
          type: 'number_range',
          value: 2,
          min: 0,
          max: 2,
          required: true,
        },
        architectures: {
          title: 'Architectures',
          hint: 'The architectures supported by the function',
          type: 'string_select',
          value: 'x86_64',
          selection: ['x86_64', 'arm64'],
          required: true,
        },
        runtime: {
          title: 'Runtime',
          hint: 'The runtime for the function',
          type: 'string_select',
          value: 'python3.8',
          selection: [
            'nodejs',
            'nodejs4.3',
            'nodejs6.10',
            'nodejs8.10',
            'nodejs10.x',
            'nodejs12.x',
            'nodejs14.x',
            'nodejs16.x',
            'java8',
            'java8.al2',
            'java11',
            'python2.7',
            'python3.6',
            'python3.7',
            'python3.8',
            'python3.9',
            'dotnetcore1.0',
            'dotnetcore2.0',
            'dotnetcore2.1',
            'dotnetcore3.1',
            'dotnet6',
            'dotnet8',
            'nodejs4.3-edge',
            'go1.x',
            'ruby2.5',
            'ruby2.7',
            'provided',
            'provided.al2',
            'nodejs18.x',
            'python3.10',
            'java17',
            'ruby3.2',
            'python3.11',
            'nodejs20.x',
            'provided.al2023',
            'python3.12',
            'java21',
          ],
          required: true,
        },
      },
      deploy: {
        filename: {
          title: 'Filename',
          hint: "Path to the function's deployment package within the local filesystem",
          type: 'string',
          value: 'index.js',
          required: true,
        },
        handler: {
          title: 'Handler',
          hint: 'The function entry point',
          type: 'string',
          value: 'index.handler',
          required: true,
        },
        authorization_type: {
          title: 'Authorization Type',
          hint: 'The type of authentication that the function URL uses. Set to "AWS_IAM" to restrict access to authenticated IAM users only. Set to "NONE" to bypass IAM authentication and create a public endpoint',
          type: 'string_select',
          value: 'NONE',
          selection: ['NONE', 'AWS_IAM'],
          required: true
        },
      },
      neighbors: [],
    },
    'NICE-EnginFrame': {
      id: 'NICE-EnginFrame',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'EC2-Auto-Scaling': {
      id: 'EC2-Auto-Scaling',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Thinkbox-Stoke': {
      id: 'Thinkbox-Stoke',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Wavelength: {
      id: 'Wavelength',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Thinkbox-XMesh': {
      id: 'Thinkbox-XMesh',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'VMware-Cloud-on-AWS': {
      id: 'VMware-Cloud-on-AWS',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Bottlerocket: {
      id: 'Bottlerocket',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Thinkbox-Sequoia': {
      id: 'Thinkbox-Sequoia',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Genomics-CLI': {
      id: 'Genomics-CLI',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'SimSpace-Weaver': {
      id: 'SimSpace-Weaver',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Lightsail: {
      id: 'Lightsail',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    EC2: {
      id: 'EC2',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Fargate: {
      id: 'Fargate',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Elastic-Beanstalk': {
      id: 'Elastic-Beanstalk',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Serverless-Application-Repository': {
      id: 'Serverless-Application-Repository',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Application-Auto-Scaling': {
      id: 'Application-Auto-Scaling',
      category: 'Compute',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
  },
  'Cloud-Financial-Management': {
    Budgets: {
      id: 'Budgets',
      category: 'Cloud-Financial-Management',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Billing-Conductor': {
      id: 'Billing-Conductor',
      category: 'Cloud-Financial-Management',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Cost-Explorer': {
      id: 'Cost-Explorer',
      category: 'Cloud-Financial-Management',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Reserved-Instance-Reporting': {
      id: 'Reserved-Instance-Reporting',
      category: 'Cloud-Financial-Management',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Savings-Plans': {
      id: 'Savings-Plans',
      category: 'Cloud-Financial-Management',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Application-Cost-Profiler': {
      id: 'Application-Cost-Profiler',
      category: 'Cloud-Financial-Management',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Cost-and-Usage-Report': {
      id: 'Cost-and-Usage-Report',
      category: 'Cloud-Financial-Management',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
  },
  Satellite: {
    'Ground-Station': {
      id: 'Ground-Station',
      category: 'Satellite',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
  },
  Blockchain: {
    'Quantum-Ledger-Database': {
      id: 'Quantum-Ledger-Database',
      category: 'Blockchain',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Managed-Blockchain': {
      id: 'Managed-Blockchain',
      category: 'Blockchain',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
  },
  'Management-Governance': {
    'Personal-Health-Dashboard': {
      id: 'Personal-Health-Dashboard',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Resilience-Hub': {
      id: 'Resilience-Hub',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    CloudWatch: {
      id: 'CloudWatch',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Resource-Explorer': {
      id: 'Resource-Explorer',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Proton: {
      id: 'Proton',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Well-Architected-Tool': {
      id: 'Well-Architected-Tool',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    AppConfig: {
      id: 'AppConfig',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'License-Manager': {
      id: 'License-Manager',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Systems-Manager': {
      id: 'Systems-Manager',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Fault-Injection-Simulator': {
      id: 'Fault-Injection-Simulator',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Managed-Grafana': {
      id: 'Managed-Grafana',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    CloudFormation: {
      id: 'CloudFormation',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Config: {
      id: 'Config',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Organizations: {
      id: 'Organizations',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Managed-Service-for-Prometheus': {
      id: 'Managed-Service-for-Prometheus',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Launch-Wizard': {
      id: 'Launch-Wizard',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Service-Management-Connector': {
      id: 'Service-Management-Connector',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    CloudTrail: {
      id: 'CloudTrail',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Service-Catalog': {
      id: 'Service-Catalog',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    OpsWorks: {
      id: 'OpsWorks',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Chatbot: {
      id: 'Chatbot',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Distro-for-OpenTelemetry': {
      id: 'Distro-for-OpenTelemetry',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Backint-Agent': {
      id: 'Backint-Agent',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Trusted-Advisor': {
      id: 'Trusted-Advisor',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Management-Console': {
      id: 'Management-Console',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Control-Tower': {
      id: 'Control-Tower',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Auto-Scaling': {
      id: 'Auto-Scaling',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Application-Auto-Scaling': {
      id: 'Application-Auto-Scaling',
      category: 'Management-Governance',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
  },
  'Media-Services': {
    'Elemental-MediaConnect': {
      id: 'Elemental-MediaConnect',
      category: 'Media-Services',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Elemental-MediaStore': {
      id: 'Elemental-MediaStore',
      category: 'Media-Services',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Elemental-Link': {
      id: 'Elemental-Link',
      category: 'Media-Services',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Elemental-Live': {
      id: 'Elemental-Live',
      category: 'Media-Services',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Elemental-MediaConvert': {
      id: 'Elemental-MediaConvert',
      category: 'Media-Services',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Elemental-Conductor': {
      id: 'Elemental-Conductor',
      category: 'Media-Services',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Elemental-MediaPackage': {
      id: 'Elemental-MediaPackage',
      category: 'Media-Services',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Kinesis-Video-Streams': {
      id: 'Kinesis-Video-Streams',
      category: 'Media-Services',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Elemental-MediaTailor': {
      id: 'Elemental-MediaTailor',
      category: 'Media-Services',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Elastic-Transcoder': {
      id: 'Elastic-Transcoder',
      category: 'Media-Services',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Nimble-Studio': {
      id: 'Nimble-Studio',
      category: 'Media-Services',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Elemental-Server': {
      id: 'Elemental-Server',
      category: 'Media-Services',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Elemental-Appliances-&-Software': {
      id: 'Elemental-Appliances-&-Software',
      category: 'Media-Services',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Elemental-MediaLive': {
      id: 'Elemental-MediaLive',
      category: 'Media-Services',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Interactive-Video-Service': {
      id: 'Interactive-Video-Service',
      category: 'Media-Services',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Elemental-Delta': {
      id: 'Elemental-Delta',
      category: 'Media-Services',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
  },
  'Machine-Learning': {
    Neuron: {
      id: 'Neuron',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Polly: {
      id: 'Polly',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Personalize: {
      id: 'Personalize',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Elastic-Inference': {
      id: 'Elastic-Inference',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    CodeWhisperer: {
      id: 'CodeWhisperer',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Rekognition: {
      id: 'Rekognition',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Monitron: {
      id: 'Monitron',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Lookout-for-Equipment': {
      id: 'Lookout-for-Equipment',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Panorama: {
      id: 'Panorama',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Transcribe: {
      id: 'Transcribe',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'SageMaker-Studio-Lab': {
      id: 'SageMaker-Studio-Lab',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    CodeGuru: {
      id: 'CodeGuru',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Lex: {
      id: 'Lex',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Translate: {
      id: 'Translate',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Forecast: {
      id: 'Forecast',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Textract: {
      id: 'Textract',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'SageMaker-Ground-Truth': {
      id: 'SageMaker-Ground-Truth',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Apache-MXNet-on-AWS': {
      id: 'Apache-MXNet-on-AWS',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Augmented-AI-A2I': {
      id: 'Augmented-AI-A2I',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    DeepRacer: {
      id: 'DeepRacer',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Comprehend-Medical': {
      id: 'Comprehend-Medical',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    SageMaker: {
      id: 'SageMaker',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Lookout-for-Vision': {
      id: 'Lookout-for-Vision',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'TensorFlow-on-AWS': {
      id: 'TensorFlow-on-AWS',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Deep-Learning-Containers': {
      id: 'Deep-Learning-Containers',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Lookout-for-Metrics': {
      id: 'Lookout-for-Metrics',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    HealthLake: {
      id: 'HealthLake',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Deep-Learning-AMIs': {
      id: 'Deep-Learning-AMIs',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Fraud-Detector': {
      id: 'Fraud-Detector',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Kendra: {
      id: 'Kendra',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Omics: {
      id: 'Omics',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    TorchServe: {
      id: 'TorchServe',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    DeepComposer: {
      id: 'DeepComposer',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    DeepLens: {
      id: 'DeepLens',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'DevOps-Guru': {
      id: 'DevOps-Guru',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Comprehend: {
      id: 'Comprehend',
      category: 'Machine-Learning',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
  },
  'Quantum-Technologies': {
    Braket: {
      id: 'Braket',
      category: 'Quantum-Technologies',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
  },
  'Migration-Transfer': {
    'Application-Discovery-Service': {
      id: 'Application-Discovery-Service',
      category: 'Migration-Transfer',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Server-Migration-Service': {
      id: 'Server-Migration-Service',
      category: 'Migration-Transfer',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Migration-Evaluator': {
      id: 'Migration-Evaluator',
      category: 'Migration-Transfer',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Transfer-Family': {
      id: 'Transfer-Family',
      category: 'Migration-Transfer',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Application-Migration-Service': {
      id: 'Application-Migration-Service',
      category: 'Migration-Transfer',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Migration-Hub': {
      id: 'Migration-Hub',
      category: 'Migration-Transfer',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    DataSync: {
      id: 'DataSync',
      category: 'Migration-Transfer',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Mainframe-Modernization': {
      id: 'Mainframe-Modernization',
      category: 'Migration-Transfer',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
  },
  'Application-Integration': {
    AppFlow: {
      id: 'AppFlow',
      category: 'Application-Integration',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    MQ: {
      id: 'MQ',
      category: 'Application-Integration',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Console-Mobile-Application': {
      id: 'Console-Mobile-Application',
      category: 'Application-Integration',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Managed-Workflows-for-Apache-Airflow': {
      id: 'Managed-Workflows-for-Apache-Airflow',
      category: 'Application-Integration',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'API-Gateway': {
      id: 'API-Gateway',
      category: 'Application-Integration',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    AppSync: {
      id: 'AppSync',
      category: 'Application-Integration',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Step-Functions': {
      id: 'Step-Functions',
      category: 'Application-Integration',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Simple-Queue-Service': {
      id: 'Simple-Queue-Service',
      category: 'Application-Integration',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Simple-Notification-Service': {
      id: 'Simple-Notification-Service',
      category: 'Application-Integration',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    EventBridge: {
      id: 'EventBridge',
      category: 'Application-Integration',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Express-Workflows': {
      id: 'Express-Workflows',
      category: 'Application-Integration',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
  },
  'Front-End-Web-Mobile': {
    'Device-Farm': {
      id: 'Device-Farm',
      category: 'Front-End-Web-Mobile',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Amplify: {
      id: 'Amplify',
      category: 'Front-End-Web-Mobile',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Location-Service': {
      id: 'Location-Service',
      category: 'Front-End-Web-Mobile',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
  },
  'Customer-Enablement': {
    'Training-Certification': {
      id: 'Training-Certification',
      category: 'Customer-Enablement',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Managed-Services': {
      id: 'Managed-Services',
      category: 'Customer-Enablement',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Support: {
      id: 'Support',
      category: 'Customer-Enablement',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    IQ: {
      id: 'IQ',
      category: 'Customer-Enablement',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    'Professional-Services': {
      id: 'Professional-Services',
      category: 'Customer-Enablement',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    rePost: {
      id: 'rePost',
      category: 'Customer-Enablement',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
    Activate: {
      id: 'Activate',
      category: 'Customer-Enablement',
      description: '',
      enabled: true,
      cost: {},
      config: {},
      deploy: {},
      neighbors: [],
    },
  },
};

export default AWS_SCHEMAS;
